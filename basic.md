...
### 1️⃣ 실시간 주가 정보 연동 (Real-time Stock Data)

**목표:** 워치리스트와 재무 페이지에 실시간 주가 자동 업데이트

#### 기능 구현
- **실시간 주가 표시**
  - 현재가, 등락률, 거래량 표시
  - 30초 자동 업데이트 (Polling 방식)
  - 등락에 따른 색상 변화 (상승: 빨강, 하락: 파랑)
  - 마지막 업데이트 시간 표시

- **데이터 소스**
  - 한국투자증권 OpenAPI (우선)
  - 네이버 금융 크롤링 (대체)
  - Yahoo Finance API (글로벌)

- **기술 스택**
  - Polling: setInterval + fetch
  - WebSocket: Socket.io (추후 업그레이드)
  - 캐싱: localStorage + Map

#### 구현 단계
1. API 조사 및 선택 (1일)
2. realtime-stock.js 구현 (2-3일)
3. UI 업데이트 로직 (1-2일)
4. 테스트 및 최적화 (1일)

---

### 2️⃣ 성능 최적화 (Performance Optimization)

**목표:** 대량 데이터 효율적 렌더링 및 빠른 응답성

#### 기능 구현
- **Virtual Scrolling (가상 스크롤)**
  - 뉴스 피드: 수백 개 뉴스를 빠르게 표시
  - 공시 리스트: 보이는 영역만 렌더링
  - 워치리스트: 많은 기업도 부드럽게

- **Lazy Loading (지연 로딩)**
  - Chart.js 차트: 스크롤 시 로드
  - 이미지: Intersection Observer 활용
  - 재무 데이터: 필요할 때만 로드

- **데이터 캐싱**
  - localStorage: 재무 데이터 (TTL 5분)
  - 메모리 캐시: 주가 정보
  - 중복 API 호출 방지

#### 구현 단계
1. Virtual Scroller 클래스 구현 (2일)
2. Intersection Observer 적용 (1일)
3. 캐싱 시스템 구축 (1일)

---

### 3️⃣ 관심 기업 모니터링 (Watchlist) ✅ **완료**
...
### ✅ 완료 (2025-11-26)
- [x] 워치리스트 시스템 (CRUD, 태그, 메모, 알림)
- [x] 투자 지표 계산 (PER, PBR, PSR, EV/EBITDA 등 8개)
- [x] 재무 건전성 점수 (100점 만점, 4개 카테고리)
- [x] 뉴스 API 연동 (Naver, DART)
- [x] 주식 API 통합 (주가, 시가총액)
- [x] 프록시 서버 구축 (Node.js/Express)
