...
const NewsAPI = {
     ...
     */
    async searchNaverNews(query, display = 5) {
        try {
            // 백엔드 프록시를 통한 호출
            ...
            if (!data.items || data.items.length === 0) {
                console.log('검색 결과가 없습니다. 샘플 데이터를 표시합니다.');
                return this.generateSampleNews(query);
            }
            
            console.log(`✅ ${data.items.length}개의 실제 뉴스를 불러왔습니다.`);
            return this.parseNaverNews(data.items);
        } catch (error) {
            console.error('네이버 뉴스 검색 실패:', error.message);
            console.log('프록시 서버가 실행 중인지 확인하세요. 샘플 데이터를 표시합니다.');
            return this.generateSampleNews(query);
        }
    },
    ...
    generateSampleNews(companyName) {
        ...
        return [
            {
                title: `${companyName}, 2025년 4분기 실적 호조 전망`,
